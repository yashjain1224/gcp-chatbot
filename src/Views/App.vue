    ...
    computed: {
        ...
        suggestions(){
            if (this.messages.length > 0){
                ...
                const suggestions = []
                console.log("9999999999999999",last_message)
                /* Dialogflow Suggestions */
                const word = "Cannot display response in Dialogflow simulator. Please test on the Google Assistant simulator instead."
                if(last_message.queryResult.fulfillmentText != word){
                ...
                if (last_message.queryResult.webhookPayload && last_message.queryResult.webhookPayload.google){
                    console.log("-----------------22",last_message.queryResult.webhookPayload.google.richResponse.items[0].simpleResponse)
                    if(last_message.queryResult.webhookPayload.google.richResponse.items[0].simpleResponse){
                        last_message.queryResult.fulfillmentMessages = ""
                    }
                    for (const component in last_message.queryResult.webhookPayload.google){
                        ...
                        console.log("--------p",suggestions.text_suggestions)
                        ...
                        console.log("---------------q",suggestions.link_suggestion)
                    }
                }
                
                console.log("-------3",suggestions,"------4",this.config.start_suggestions)
                return suggestions
            }
            return {
    ...
    methods: {
        send(submission){
            ...
            this.$http.post ( queryResURL, JSON.stringify(request) ).then(function (queryResponse) {
                ...
                // if(queryResponse.body.queryResult.fulfillmentText == word){
                //   console.log("--------dont do anything")
                // }
                // else {
                this.messages.push(queryResponse.body)
                console.log("00000000000",this.messages)
                this.handle(queryResponse.body)
        ...
        handle(response){
            ...
            if (response.outputAudio){
                ...
                const word = "Cannot display response in Dialogflow simulator. Please test on the Google Assistant simulator instead."
                if(response.queryResult.fulfillmentText != word){
                for (const component in response.queryResult.fulfillmentMessages){
                    console.log("-------0",response.queryResult.fulfillmentMessages)
                    console.log("------------1",response.queryResult.fulfillmentMessages[component].text,"======",response.queryResult.fulfillmentMessages[component].simpleResponses,"------0",response.queryResult.fulfillmentMessages[component].rbmText);
                ...
                if (response.queryResult.webhookPayload && response.queryResult.webhookPayload.google){
                    for (const component in response.queryResult.webhookPayload.google){
                         console.log("---------------2",response.queryResult.webhookPayload.google[component].simpleResponse)
